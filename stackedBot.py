...
class StackedBot(discord.Client):
	def __init__(self):
		...
		# Channels
		self.com_channels = {'guild': None, 'server': None, 'public': None, 'lobby': None}
	...
	async def on_ready(self):		
		...
		self.com_channels['guild']  = self.guilds[0].get_channel(int(os.getenv('GUILD_CHANNEL')))
		self.com_channels['server'] = self.guilds[0].get_channel(int(os.getenv('SERVER_CHANNEL')))
		self.com_channels['public'] = self.guilds[0].get_channel(int(os.getenv('PUBLIC_CHANNEL')))
		self.com_channels['lobby'] = self.guilds[0].get_channel(int(os.getenv('LOBBY_CHANNEL')))

		for channel in self.com_channels:
			if self.com_channels[channel] == None:
	...
	def setup_notifications(self):
		...
		self.add_notification('30 11,17,20 * * * 0', self.com_channels['public'], 'Energy to be claimed! Go go!')
		
		self.add_notification('0 12 * * 1,4 0',  self.com_channels['public'], 'Dragon is invading! Remember to fix ballista!')
		self.add_notification('30 20 * * 1,4 0', self.com_channels['public'], 'Dragon is leaving in 30 minutes! Remember to fix ballista!')
		
		self.add_notification('0 21 * * * 0', self.com_channels['public'], 'Guild reward packs! Go claim some lewt!')
		
		self.add_notification('45 20 * * * 0', self.com_channels['public'], '15 minutes to arena rewards')
		
		self.add_notification('15 21 * * * 0', self.com_channels['guild'], '15 minutes to underground rewards! Go get em castles :partying_face:')
		
		self.add_notification('0 5 * * 3 0', self.com_channels['public'], 'Sphinx is coming today, save up some movement!')
		self.add_notification('0 9 * * 3 0', self.com_channels['public'], 'Sphinx is here, go play trivia!')
		
		# KvK
		self.add_notification('45 8 * * 3 0',    self.com_channels['server'], f'15 minutes to KvK starts!! {self.everyone}')
		self.add_notification('45 8 * * 4,5 0',  self.com_channels['server'],  '15 minutes till todays KvK rounds start!')
		self.add_notification('45 21 * * 3,4 0', self.com_channels['server'],  '15 minutes to KvK rewards! Go get em Kingdoms :partying_face:')
		self.add_notification('45 21 * * 5 0',   self.com_channels['server'],  '15 minutes to KvK ends! Go get em Kingdoms :partying_face:')
		
		# BoG
		self.add_notification('45 19 * * 2 0', self.com_channels['public'], '15 minutes to group game in BoG! Remember rosters!', StackedBot.qualifying_week)
		self.add_notification('45 19 * * 3 0', self.com_channels['public'], '15 minutes to Battle of Gods quarter finals! Go place your bets and rosters :partying_face:', StackedBot.qualifying_week)
		self.add_notification('45 19 * * 4 0', self.com_channels['public'], '15 minutes to Battle of Gods finals! Go place your bets and rosters :partying_face:', StackedBot.qualifying_week)
		
		# Endless inferno
		self.add_notification('0 9 * * 2,5 0', self.com_channels['public'], '"Endless" inferno is here, go climb the ladder!')
		self.add_notification('30 11 * * 2,5 0', self.com_channels['public'], '"Endless" inferno refresh in 30 minutes!')
		...
		# Premium Cards
		self.add_notification('0 5 1 * * 0', self.com_channels['public'], f'New premium deck out! Activate it BEFORE starting dailies! {self.everyone}')

	# Wrapper for adding notifications
	def add_notification(self, time, channel, message, active=None):
		self.cronTab.append(aiocron.crontab(time, func=partial(StackedBot.send_notification, self, channel, message, active), start=True, tz=timezone(timedelta(hours=1))))
	...
	async def send_notification(self, channel, msg, active=None):
		if active != None and active(self) and channel != None:
			await channel.send(f'{msg}')
		elif active == None and channel != None:
			await channel.send(f'{msg}')
		elif active == None and channel == None:
	...
	async def on_raw_reaction_add(self, reactionEvent):
		...
		try:		
			...
			await message.channel.send(f'"{message.content}" in {translated.dest} is: ```{translated.text}```')
		except:
	...
	async def on_member_join(self, member):
		await self.com_channels['lobby'].send(f'Hi {member.mention}, welcome to Stacked! :partying_face:')
	...
	def chatbot_process(self, message, reply=False):
		...
		return self.bot.get_response(msg.replace(' 2.0 - Devil\'s end', ' '))
	...
	def handle_command(self, msg, full_message):
		...
		elif msg.startswith('!addis') or msg.startswith('!whatis') or msg.startswith('!remis'):  
	...
	def help(self, message):
		...
		response+= '!kvkcalc <OurCurrent> <TheirCurrent> <OurGain1> <TheirGain1> <OurGain2> <TheirGain2> <OurGain3> <TheirGain3> - See if we win KvK\n'
		response+= 'React to a message with your flag, and I\'ll translate that for you\n'
	...
	def kvk_calc(self, message):
		...
		if len(components) != 9:
			return "Usage: !kvkcalc <OurCurrent> <TheirCurrent> <OurGain1> <TheirGain1> <OurGain2> <TheirGain2> <OurGain3> <TheirGain3>"
		try:
	...
	def events_message(self, msg):
		...
		if not daily:
			if qualifying_week == 1:
				response += 'Qualifying week for Battle of the Gods!\n'
			else:
				response += 'Battle of the Gods!\n'
		
		if daily and not qualifying_week:
			if day_of_week == 2:
				response += 'Group game in Battle of the Gods\n'
			elif day_of_week == 3:
				response += 'Quarter finals in Battle of the Gods!\n'
			elif day_of_week == 4:
				response += 'Final battles of the Gods!\n'
				
		if daily and day_of_week in [1,4]:
			response += 'Dragon invasion\n'
		elif not daily:
			response += 'Monday and Thursdag - Dragon invasion\n'
